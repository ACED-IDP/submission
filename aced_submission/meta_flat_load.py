...
def create_index_from_source(_schema, _index, _type):
    ...
    return {
        ...
        "mappings": {
            _type: {"properties": mappings, "dynamic_templates": dynamic_templates}
        }
    }


def write_array_aliases(
    doc_type, alias, elastic=DEFAULT_ELASTIC, name_space=ES_INDEX_PREFIX
):
    ...
    return {
        ...
        "json": {
            "actions": [
                {
                    "add": {
                        "index": f"{name_space}_{doc_type}-array-config_0",
...
def write_array_config(
    doc_type, alias, field_array, elastic=DEFAULT_ELASTIC, name_space=ES_INDEX_PREFIX
):
...
def write_alias_config(
    ...
    return {
        ...
        "json": {
            "actions": [
                {"add": {"index": f"{name_space}_{doc_type}_0", "alias": alias}}
            ]
        },
    }
...
def write_sqlite(index, generator):
    ...
    with connection:
        ...
        connection.execute(
            f"CREATE TABLE if not exists {index} (id PRIMARY KEY, entity Text)"
        )
        with connection:
...
def write_bulk_http(elastic, index, limit, doc_type, generator, schema):
    ...
    _ = bulk(
        client=elastic, actions=(d for d in _bulker(generator)), request_timeout=120
    )
...
def observation_generator(project_id, path) -> Iterator[Dict]:
    ...
    for observation in read_ndjson(path):
        ...
        if patient:
            ...
            o_["address_orh_zip_designation_code"] = patient.get(
                "address_orh_zip_designation_code", None
            )
...
def fetch_denormalized_patient(connection, patient_id):
    ...
    for row in connection.execute(
        "select entity from patient where id = ? limit 1", (patient_id,)
    ):
        patient = orjson.loads(row[0])
        break

    for row in connection.execute(
        "select entity from family_history where patient_id = ? ", (patient_id,)
    ):
    ...
    for row in connection.execute(
        "select entity from condition where patient_id = ? ", (patient_id,)
    ):
    ...
    return {
        ...
        "fh_condition": fh_condition,
        "fh_condition_coding": fh_condition_coding,
    }
...
def setup_aliases(alias, doc_type, elastic, field_array, index):
    ...
    elastic.indices.update_aliases(
        {
            "actions": [
                {"add": {"index": f"{ES_INDEX_PREFIX}_{doc_type}_0", "alias": alias}}
            ]
        }
    )
    elastic.indices.update_aliases(
        {
            "actions": [
                {
                    "add": {
                        "index": f"{ES_INDEX_PREFIX}_{doc_type}-array-config_0",
...
def denormalize_patient(input_path):
    ...
    with connection:
        ...
        connection.execute(
            "CREATE TABLE if not exists patient (id PRIMARY KEY, entity Text)"
        )
        connection.execute(
            "CREATE TABLE if not exists family_history (id PRIMARY KEY, patient_id Text, entity Text)"
        )
        connection.execute(
            "CREATE TABLE if not exists condition (id PRIMARY KEY, patient_id Text, entity Text)"
        )
    with connection:
        connection.executemany(
            ...
            [
                ...
                for entity in _load_vertex("Patient.ndjson")
            ],
        )
    with connection:
        connection.executemany(
            ...
            [
                ...
                for entity in _load_vertex("FamilyMemberHistory.ndjson")
            ],
        )
    with connection:
        connection.executemany(
            ...
            [
                ...
                for entity in _load_vertex("Condition.ndjson")
            ],
        )
    with connection:
        connection.execute(
            "CREATE INDEX if not exists condition_patient_id on condition(patient_id)"
        )
        connection.execute(
            "CREATE INDEX if not exists family_history_patient_id on condition(patient_id)"
        )
...
@click.option(
    "--index", required=True, default=None, show_default=True, help="Elastic index name"
)
@click.option(
    "--path",
    required=True,
...
@click.option(
    "--limit", default=None, show_default=True, help="Max number of rows per index."
)
@click.option(
    "--schema_path",
    required=True,
...
def load_flat(project_id, index, path, limit, elastic_url, schema_path, output_path):
    ...
    if index == "patient":
        ...
        field_array = [
            k
            for k, v in schema["patient.yaml"]["properties"].items()
            if "array" in v.get("type", {})
        ]
    ...
    if index == "observation":
        ...
        field_array = [
            k
            for k, v in schema["observation.yaml"]["properties"].items()
            if "array" in v.get("type", {})
        ]
    ...
    if index == "file":
        ...
        field_array = [
            k
            for k, v in schema["document_reference.yaml"]["properties"].items()
            if isinstance(v, dict) and "array" in v.get("type", {})
        ]
        if not output_path:
            # create the index and write data into it.
            write_bulk_http(
...
def counts(project_id):
    ...
    query = {
        "query": {
            "match": {"auth_resource_path": f"/programs/{program}/projects/{project}"}
        }
    }
