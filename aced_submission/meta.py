...
from aced_submission.meta_graph_load import (
    meta_upload,
    _table_mappings,
...
@click.option(
    "--project_id", required=True, show_default=True, help="Gen3 program-project"
)
@click.option(
    "--credentials_file",
...
def _meta_upload(
    source_path, project_id, credentials_file, silent, dictionary_path, config_path
):
...
def _node_counts(config_path, output_format):
    ...
    assert pathlib.Path(
        config_path
    ).exists(), f"--config_path {config_path} does not exist"
    dependency_order = yaml.safe_load(open(config_path))["dependency_order"]
    dependency_order = [
        _
        for _ in dependency_order
        if not _.startswith("_") and _ not in ["Project", "Program"]
    ]
...
def _graph_rm(config_path, project_id, output_format):
    ...
    reverse_dependency_order = [
        _
        for _ in reverse_dependency_order
        if not _.startswith("_") and _ not in ["Project", "Program"]
    ]
    reverse_dependency_order = [
        (rank + 1, _) for rank, _ in enumerate(reverse_dependency_order)
    ]
        ...
        with conn.cursor() as curs:
            for rank, _ in reverse_dependency_order:
                if _.startswith("_"):
                    continue
                curs.execute(
                    f"delete from node_{_.lower()} where _props->>'project_id' = %s;",
                    (project_id,),
                )
                results.append(
                    {
                        "table": f"node_{_.lower()}",
                        "project_id": project_id,
                        "count": curs.rowcount,
                    }
                )
            curs.execute(
                "delete from node_project where _props->>'code' = %s;", (project,)
            )
            results.append(
                {"table": "node_project", "project_id": project, "count": curs.rowcount}
            )
...
@click.option("--program", default="aced", show_default=True, help='Gen3 "program"')
@click.option(
    "--credentials_file",
    default="~/.gen3/credentials.json",
    show_default=True,
