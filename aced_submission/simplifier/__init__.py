...
import aced_submission.util
from aced_submission import EmitterContextManager
from aced_submission.simplifier.oid_lookup import get_oid
...
def _gen3_scaffolding_document_reference(simplified: dict, resource: FHIRAbstractModel) -> dict:
    ...
    assert 'content_url' in simplified, \
        f"Missing content_url in simplified {simplified['resourceType']} {simplified['id']}"
...
def _render_dialect(simplified: dict, references: List[str], dialect: str) -> dict:
    ...
    for _ in references:
        label, id_ = _.split('/')
        gen3_links.append({"dst_id": id_, "dst_name": label})

    return {'id': simplified['id'], 'name': simplified['resourceType'], 'relations': gen3_links, 'object': simplified}
...
def simplify_directory(input_path, pattern, output_path, schema_path, dialect):
    ...
    with SimplifierContextManager():
        with EmitterContextManager(output_path, verbose=True) as emitter:
            for parse_result in aced_submission.util.directory_reader(directory_path=input_path, pattern=pattern,
                                                                      validate=False):
                if parse_result.exception is not None:
                    if 'resourceType' not in str(parse_result.exception):
                        logger.warning(f"{parse_result.path} has exception {parse_result.exception}")
                    continue
                ...
                simplified = _render_dialect(simplified, references, dialect)
...
@click.option('--input_path', required=True,
              default='tests/fixtures/simplify/',
              show_default=True,
...
@click.option('--output_path', required=True,
              default=None,
              show_default=True,
              help='Path where simplified resources will be stored'
              )
...
def cli(input_path, pattern, output_path, schema_path, dialect):
    """Renders Gen3 friendly flattened records."""
    simplify_directory(input_path, pattern, output_path, schema_path, dialect)
