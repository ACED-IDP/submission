...
def _validate(gen3_resource: dict, schemas: dict):
    ...
    try:
        ...
        for expected_property in ["id", "relations", "object"]:
            assert (
                expected_property in gen3_resource
            ), f"{expected_property} not in {gen3_resource}"
        ...
        expected_keys = set(schema["properties"].keys()).union(
            set([_["name"] for _ in schema["links"]])
        )
        if not actual_keys.issubset(expected_keys):
            if not schema.get("additionalProperties", False):
                assert (
                    False
                ), f"Is not a subset {actual_keys - expected_keys} not expected"
            else:
